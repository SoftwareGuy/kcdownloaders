...
def get_standardSWF():
	...
	resourceSwfFiles = [ "commonAssets.swf", "font.swf", "icons.swf", "sound_bgm.swf", "sound_se.swf" ]		# Core game resources
	sceneSwfFiles = [ "AlbumMain.swf", "ArsenalMain.swf", "DutyMain.swf", "InteriorMain.swf", "ItemlistMain.swf", "OrganizeMain.swf", "RecordMain.swf", "RemodelMain.swf", "RepairMain.swf", "SallyMain.swf", "SupplyMain.swf", "TitleMain.swf", "tutorial.swf" ]	# All the scenes I can remember.
...
def get_shipGirlGFX(filename):
	...
	for ship in shipGFX:
		with open("kcs/resources/swf/ships/" + ship['api_filename'] + ".swf", 'wb') as outFile:
			print("Getting ship graphics. API ID: " + str(count) + ", file name: " + ship['api_filename'] + ", shipgirl ID " + str(ship['api_sortno']) + ".");
			c = pycurl.Curl()
